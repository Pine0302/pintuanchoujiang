...
$filed = 'id,status,name,type,group_size,user_level,number,createtime,luck_draw_num';
$condition = array('customer_id'=>$customer_id,'isvalid'=>true);
...
$dcount = $collageActivities->getActivitiesMes($condition,'count(1) AS dcount');
//分页---start
$pagenum = 1;
$pagesize = 20;
$begintime="";
...
$page=ceil($rcount_q/$end);
 ...
 /* 输出数量结束 */
$condition['ORDER'] = " ORDER BY createtime DESC";
$condition['LIMIT'] = " LIMIT ".$start.",".$end."";
$list = $collageActivities->getActivitiesMes($condition,$filed);
  ...
  <div  class="WSY_data">
	 <!--列表按钮开始-->
      <div class="WSY_list" id="WSY_list">
		...
		<table class="WSY_t2"  style="border: 1px solid #D8D8D8;border-collapse: collapse;">
			...
			<?php
				foreach($list['data'] as $key => $val){
					$filed = 'sum(pt.total_open) AS open_count,sum(pt.total_success) AS success_count,sum(pt.total_fail) AS fail_count,sum(pt.total_conduct) AS conduct_count';
					$condition = array(
									'pt.activitie_id' => $val['id'],
									'pt.isvalid' => true,
                                    'ae.customer_id' => $customer_id,
                                    'ae.isvalid' => true,
									'at.customer_id' => $customer_id
								);
					$ccount = $collageActivities->get_activities_product($condition,$filed);
					$query = "SELECT sum(total_price) as all_total_price FROM collage_group_order_t where isvalid=true AND activitie_id=".$val['id']." LIMIT 1";
					$result = _mysql_query($query) or die('fail:'.mysql_error());
					while($row = mysql_fetch_object($result)){
						$all_total_price = empty($row->all_total_price)?0:$row->all_total_price;
					}
			...
			?>
				<tr style="border:1px solid #D8D8D8">
					...
					<td><?php echo $num = empty($ccount['data'][0]['open_count'])?0:$ccount['data'][0]['open_count'];?></td>
					<td><?php echo $num = empty($ccount['data'][0]['success_count'])?0:$ccount['data'][0]['success_count'];?></td>
					<td><?php echo $num = empty($ccount['data'][0]['fail_count'])?0:$ccount['data'][0]['fail_count'];?></td>
					<td><?php echo $num = empty($ccount['data'][0]['conduct_count'])?0:$ccount['data'][0]['conduct_count'];?></td>
					<td><?php echo round($all_total_price,2);?></td>
					<td class="WSY_remind_main">
						<a href="proActivityList.php?keyid=<?php echo $val['id'];?>" style="cursor:pointer;" >
							<span class="operation-btn WSY-skin-bg">产品活动列表</span>
						</a>
					</td>
				</tr>
			<?PHP }?>
		...
		<script type="text/javascript">
			...
			$(".WSY_page").createPage({
				...
				current:pagenum,
				backFn:function(p){
				 document.location= "activityMes.php?customer_id="+customer_id+"&keyid=<?php echo $keyid; ?>&pagenum="+p+"&search_id="+search_id+"&search_name="+search_name+"&search_type="+search_type+"&search_status="+search_status;
			   }
			});
		    ...
		    function jumppage(){
				...
				if((a<1) || (a==pagenum) || (a>page) || isNaN(a)){
					...
					document.location= "activityMes.php?customer_id="+customer_id+"&keyid=<?php echo $keyid; ?>&pagenum="+a+"&search_id="+search_id+"&search_name="+search_name+"&search_type="+search_type+"&search_status="+search_status;
				}
		    }
		...
		$('#my_search').click(function(){
			...
			var url = "activityMes.php?customer_id="+customer_id+"&keyid=<?php echo $keyid; ?>";
