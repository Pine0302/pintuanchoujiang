...
$filed = 'DISTINCT wu.id,wu.name as user_name,wu.weixin_name as user_weixin_name,wu.phone,wu.weixin_headimgurl,wu.createtime,wu2.name as parent_name,wu2.weixin_name as parent_weixin_name,mt.total_partakep,mt.total_open,mt.total_success,mt.total_fail';

$condition = array('wu.customer_id'=>$customer_id,'wu.isvalid'=>true,'mt.isvalid'=>true,'cgot.type'=>5);
$search_user_id = '';//搜索ID
...
$dcount = $collageActivities->get_bbt_user($condition,'count(DISTINCT wu.id) AS dcount');
//分页---start
$pagenum = 1;
$pagesize = 20;
$begintime="";
...
$list = $collageActivities->get_bbt_user($condition,$filed);
      ...
      <div class="WSY_list" id="WSY_list">
		...
		<table class="WSY_t2"  width="97%"  style="border: 1px solid #D8D8D8;border-collapse: collapse;">
			<thead class="WSY_table_header">
				<tr style="border:none">
					...
					<th width="10%">推荐人</th>
					<th width="10%">注册时间</th>
					<th width="6%">抱抱团开团总数</th>
					<th width="6%">抱抱团参团总数</th>
					<th width="6%">开团成功团数</th>
					<th width="6%">参团成功团数</th>
					<th width="6%">获得购物币总额</th>
					<th width="6%">消费购物币总额</th>
				</tr>
			</thead>
			<tbody>
			<?php
				foreach($list['data'] as $key => $val){
                    //开团数
                    $condition = " ccot.isvalid=true AND ccot.customer_id=".$customer_id." AND ccot.is_head=1 AND ccot.user_id=".$val['id']." AND cgot.type=5";
                    $filed = " count(ccot.id) as open_nums ";
                    $open_nums = $collageActivities->select_count_crew_order($condition,$filed)['data'][0]['open_nums'];
                    //参团数
                    $condition = " ccot.isvalid=true AND ccot.customer_id=".$customer_id." AND ccot.is_head=2 AND ccot.user_id=".$val['id']." AND cgot.type=5";
                    $filed = " count(ccot.id) as join_nums ";
                    $join_nums = $collageActivities->select_count_crew_order($condition,$filed)['data'][0]['join_nums'];
                    //开团成功数
                    $condition = " ccot.isvalid=true AND ccot.customer_id=".$customer_id." AND ccot.is_head=1 AND ccot.status=5 AND ccot.user_id=".$val['id']." AND cgot.type=5";
                    $filed = " count(ccot.id) as success_open_nums ";
                    $success_open_nums = $collageActivities->select_count_crew_order($condition,$filed)['data'][0]['success_open_nums'];
                    //参团成功数
                    $condition = " ccot.isvalid=true AND ccot.customer_id=".$customer_id." AND ccot.is_head=2 AND ccot.status=5 AND ccot.user_id=".$val['id']." AND cgot.type=5";
                    $filed = " count(ccot.id) as success_join_nums ";
                    $success_join_nums = $collageActivities->select_count_crew_order($condition,$filed)['data'][0]['success_join_nums'];
                    //获得购物币总额
                    $query = "SELECT sum(wccl.cost_currency) as get_currency FROM weixin_commonshop_currency_log AS wccl
                                LEFT JOIN collage_crew_order_t AS ccot ON wccl.batchcode=ccot.batchcode
                                LEFT JOIN collage_group_order_t AS cgot ON cgot.id=ccot.group_id
                                WHERE wccl.isvalid=true AND wccl.type=1 AND wccl.customer_id=".$customer_id." AND wccl.user_id=".$val['id']." AND cgot.type=5";
                    $get_currency = 0;
                    $result = _mysql_query($query) or die('Query failed'.mysql_error());
                    while ( $row = mysql_fetch_object($result) ) {
                        $get_currency = $row->get_currency;
                    }
                    //消费购物币总额
                    $query = "SELECT sum(wccl.cost_currency) as use_currency FROM weixin_commonshop_currency_log AS wccl
                                LEFT JOIN collage_crew_order_t AS ccot ON wccl.batchcode=ccot.batchcode
                                LEFT JOIN collage_group_order_t AS cgot ON cgot.id=ccot.group_id
                                WHERE wccl.isvalid=true AND wccl.type=0 AND wccl.customer_id=".$customer_id." AND wccl.user_id=".$val['id']." AND cgot.type=5";
                    $use_currency = 0;
                    $result = _mysql_query($query) or die('Query failed'.mysql_error());
                    while ( $row = mysql_fetch_object($result) ) {
			...
			?>
				<tr style="border:1px solid #D8D8D8">
					...
					<td><?php echo $open_nums;?></td>
					<td><?php echo $join_nums;?></td>
					<td><?php echo $success_open_nums;?></td>
					<td><?php echo $success_join_nums;?></td>
					<td><?php echo bcadd($get_currency,0,2);?></td>
					<td><?php echo bcadd($use_currency,0,2);?></td>
				</tr>
			<?PHP }?>
		...
		<script type="text/javascript">
			...
			$(".WSY_page").createPage({
				...
				current:pagenum,
				backFn:function(p){
				 document.location= "currencyMes.php?customer_id="+customer_id+"&keyid=<?php echo $keyid; ?>&pagenum="+p+"&search_user_id="+search_user_id+"&search_name="+search_name+"&search_weixin_name="+search_weixin_name+"&search_phone="+search_phone;
			   }
			});
		    ...
		    function jumppage(){
				...
				if((a<1) || (a==pagenum) || (a>page) || isNaN(a)){
					...
					document.location= "currencyMes.php?customer_id="+customer_id+"&keyid=<?php echo $keyid; ?>&pagenum="+a+"&search_user_id="+search_user_id+"&search_name="+search_name+"&search_weixin_name="+search_weixin_name+"&search_phone="+search_phone;
				}
		    }
		...
		$('#my_search').click(function(){
			...
			var url = "currencyMes.php?customer_id="+customer_id+"&keyid=<?php echo $keyid; ?>";
